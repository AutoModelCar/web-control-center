...
var ControlController = function () {
  function ControlController($route, $timeout, $interval, $http, Settings, Domains) {
    var _this = this;
    ...
    this.DomainsToShow = ['app', 'manual_control', 'model_car', 'odom', 'usb_cam', 'scan', 'simple_drive_control', 'simple_parking_maneuver'];
  ...
  _createClass(ControlController, [{
    ...
    key: 'otherModeActive',
    value: function otherModeActive() {
    ...
    value: function loadData() {
      ...
      ros.getTopics(function (topics) {
        ...
        for (var i = 0; i < topics.topics.length; i += 1) {
          var topic = {};
          topic.name = topics.topics[i];
          topic.type = topics.types[i];
          _this7.data.topics.push(topic);
        }
      });
    ...
    key: 'sendToApi',
    value: function sendToApi(command) {
      var dataJson = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
  ...
  _createClass(SettingsService, [{
    ...
    value: function getDefaultSetting() {
      return {
        ...
        log: '/rosout',
        imagePreview: { port: 8080, quality: 70, width: 640, height: 480 },
        battery: false,
        batteryTopic: '',
        advanced: false
      };
...
var TopicController = function () {
  ...
  _createClass(TopicController, [{
    ...
    value: function $onInit() {
      ...
      // Check if file exists
      var fileName = '' + path + this.topic.type + '.html';
